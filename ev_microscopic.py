...
def page_home():
    ...
    with main_col:
        ...
        # ตรวจสอบว่าไฟล์รูปมีอยู่จริง หากไม่มีให้คอมเมนต์บรรทัดนี้
...
def content_symptoms():
    ...
    with col1:
        ...
        st.write("Image Placeholder for Symptoms") # ใส่ไว้เผื่อรูปไม่มี
    with col2:
...
def page_ai_detect():
    ...
    st.write("อัปโหลดภาพถ่ายจากกล้องจุลทรรศน์เพื่อทำการวิเคราะห์")

    # ส่วนที่เติม: ปุ่มอัปโหลดไฟล์ และ ตัวแปร threshold
    uploaded_file = st.file_uploader("เลือกไฟล์รูปภาพ (.jpg, .jpeg, .png)", type=['jpg', 'jpeg', 'png'])
    
    # ค่า Threshold (อาจจะปรับได้ หรือ Fix ไว้)
    ...
    if uploaded_file is not None:
        try:
            # ส่วนที่เติม: อ่านไฟล์รูปภาพเข้า OpenCV
            file_bytes = np.asarray(bytearray(uploaded_file.read()), dtype=np.uint8)
            image_bgr = cv2.imdecode(file_bytes, 1) # โหลดเป็น BGR สำหรับ OpenCV
            image_np = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB) # แปลงเป็น RGB สำหรับแสดงผลใน Streamlit
            ...
            if model is not None:
                ...
                st.warning("⚠️ Model not loaded (ev_cnn_mobile.keras not found).")
        
        # === ส่วนที่แก้ไข: เพิ่ม except block ที่หายไป ===
        except Exception as e:
...
def show_card(file_name, title, key_id):
    ...
    with st.container(border=True):
        # ตรวจสอบว่าไฟล์มีอยู่จริงหรือไม่ เพื่อกัน Error หากไม่มีรูป
...
def page_dataset():
    ...
    # ตัวอย่างการวนลูปสร้างการ์ด (ช่วยลดบรรทัดโค้ด)
    # สมมติมีรูปชื่อ pinworm_01.jpg ถึง pinworm_20.jpg
    # คุณสามารถใช้ Loop แทนการเขียนทีละบรรทัดได้ ถ้าไฟล์ชื่อเรียงกัน
    
    # แต่ใช้ตามโครงสร้างเดิมที่คุณเขียนมา เพื่อความชัวร์เรื่องชื่อไฟล์
    col1, col2, col3, col4 = st.columns(4)
    with col1: show_card("pinworm_11.jpg", "ไข่พยาธิ 01", 1)
    with col2: show_card("pinworm_12.jpg", "ไข่พยาธิ 02", 2)
    with col3: show_card("pinworm_03.jpg", "ไข่พยาธิ 03", 3)
    with col4: show_card("pinworm_04.jpg", "ไข่พยาธิ 04", 4)

    col5, col6, col7, col8 = st.columns(4)
    with col5: show_card("pinworm_05.jpg", "ไข่พยาธิ 05", 5)
    with col6: show_card("pinworm_06.jpg", "ไข่พยาธิ 06", 6)
    with col7: show_card("pinworm_07.jpg", "ไข่พยาธิ 07", 7)
    with col8: show_card("pinworm_08.jpg", "ไข่พยาธิ 08", 8)

    # ... เพิ่มส่วนที่เหลือตามต้องการ ...
