...
def page_home():
    ...
    with main_col:
        ...
        **⚠️ คำเตือน:** ผลลัพธ์จาก AI นี้ใช้เพื่อการ **คัดกรองเบื้องต้นเท่านั้น** ไม่สามารถใช้แทนการวินิจฉัยโดยแพทย์หรือนักเทคนิคการแพทย์ได้ 
        ...
        st.write("""บัณฑิตศึกษามหาวิทยาลัยวลัยลักษณ์""")
...
# --- 1. สร้างฟังก์ชันช่วยทำสวย (วางไว้บนสุดของไฟล์ หรือก่อนเรียกใช้) ---
def show_image_card(file_name, title, key_id):
    """
    ฟังก์ชันสำหรับสร้างการ์ดรูปภาพสวยๆ 1 ใบ
    - file_name: ชื่อไฟล์รูป (เช่น 'Pinworm_01.jpg')
    - title: ชื่อที่จะโชว์บนการ์ด (เช่น 'รูปที่ 1')
    - key_id: รหัสปุ่ม (ต้องไม่ซ้ำกัน ใส่เลข 1, 2, 3...)
    """
    # สร้างกรอบการ์ด (Border)
    with st.container(border=True):
        # แสดงรูปภาพ
        st.image(file_name, use_column_width=True)
        
        # จัด layout ชื่อรูปให้อยู่ตรงกลาง (ใช้ markdown ช่วย)
        st.markdown(f"<h5 style='text-align: center; color: #333;'>{title}</h5>", unsafe_allow_html=True)
        
        # ปุ่มดาวน์โหลดแบบเต็มความกว้าง (Primary Color)
        with open(file_name, "rb") as f:
            st.download_button(
                label="📥 ดาวน์โหลดรูปนี้",  # ใส่ไอคอนเพิ่มความสวย
                data=f,
                file_name=file_name,
                mime="image/jpeg",
                key=f"btn_{key_id}",      # key ห้ามซ้ำ
                use_container_width=True, # 👉 คำสั่งนี้ทำให้ปุ่มยาวเต็มกรอบ สวยมาก!
                type="primary"            # 👉 เปลี่ยนปุ่มเป็นสีแดง/สีหลักของธีม
            )
...
def page_dataset():
   ...
   col1, col2, col3 = st.columns(3)
    ...
    # --- แถวที่ 1 ---
    with col1:
        # เรียกใช้ฟังก์ชัน: (ชื่อไฟล์, ชื่อโชว์, เลขไม่ซ้ำ)
        show_image_card("IMG_20250610_142954.jpg", "ไข่พยาธิ ระยะที่ 1", 1)
        
    with col2:
        show_image_card("IMG_20250610_143236.jpg", "ไข่พยาธิ ระยะที่ 2", 2)

    with col3:
        show_image_card("IMG_20250610_143321.jpg", "ไข่พยาธิ ระยะที่ 3", 3)
