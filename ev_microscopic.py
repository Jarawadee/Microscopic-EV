...
""")
    ...
    st.write("""Centers for Disease Control and Prevention. (2024, June 7). _Schistosomiasis_. https://www.cdc.gov/dpdx/schistosomiasis/index.html""")
...
def page_ai_detect():
    ...
    st.write("อัปโหลดภาพถ่ายไข่พยาธิจากกล้องจุลทรรศน์เพื่อทำการวิเคราะห์")

    # --- ส่วนปรับค่าพารามิเตอร์ (ซ่อนใน Expander เพื่อความสวยงาม) ---
    with st.expander("⚙️ ตั้งค่าการตรวจจับ (Advanced Settings)"):
        col_set1, col_set2, col_set3 = st.columns(3)
        with col_set1:
            detection_threshold = st.slider(
                "Confidence Threshold", 
                0.0, 1.0, DEFAULT_THRESHOLD, 0.05,
                help="ค่าความมั่นใจขั้นต่ำที่จะนับว่าเป็นไข่พยาธิ"
            )
        with col_set2:
            nms_threshold = st.slider(
                "NMS Threshold", 
                0.0, 1.0, DEFAULT_NMS_THRESHOLD, 0.05,
                help="ค่าสำหรับการตัดกรอบที่ซ้อนทับกัน (Non-Maximum Suppression)"
            )
        with col_set3:
            merge_iou_threshold = st.slider(
                "Merge IoU Threshold", 
                0.0, 1.0, DEFAULT_MERGE_IOU_THRESHOLD, 0.05,
                help="ค่าสำหรับการรวมกรอบที่อยู่ติดกัน"
            )
    ...
    if uploaded_file is not None:
        try:
            # 1. แปลงไฟล์ที่อัปโหลดเป็นรูปภาพ (PIL Image)
            image_pil = Image.open(uploaded_file)
            
            # 2. แปลงเป็น Numpy Array (RGB)
            image_np = np.array(image_pil)
            
            # 3. แปลงเป็น BGR สำหรับ OpenCV (เพราะ OpenCV ใช้ BGR แต่ PIL ใช้ RGB)
            ...
            with col1:
                ...
                st.image(image_np, caption=f"Filename: {uploaded_file.name}", use_container_width=True)

            # --- เริ่มการประมวลผล ---
            if model is not None:
                # ปุ่มกดเพื่อให้เริ่มทำ (จะได้ไม่ประมวลผลทันทีตอนเลื่อน Slider)
                if st.button("🚀 เริ่มวิเคราะห์ (Analyze)", type="primary"):
                    
                    with st.spinner('กำลังวิเคราะห์ภาพด้วย AI...'):
                        # ส่งค่า threshold ต่างๆ ที่ตั้งค่าไว้เข้าไปในฟังก์ชัน
                        output_img_bgr = ObjectDet(
                            image_bgr, 
                            detection_threshold, 
                            nms_threshold, 
                            merge_iou_threshold
                    ...
                    # แปลงสีภาพผลลัพธ์กลับจาก BGR -> RGB เพื่อแสดงผลใน Streamlit
                    output_img_rgb = cv2.cvtColor(output_img_bgr, cv2.COLOR_BGR2RGB)
                    
                    with col2:
                        st.subheader("ผลการวิเคราะห์")
                        st.image(output_img_rgb, caption="AI Result", use_container_width=True)
                    
                    st.success("✅ วิเคราะห์เสร็จสิ้น")
            else:
                st.error("❌ ไม่สามารถโหลด Model ได้ (ev_cnn_mobile.keras not found).")
