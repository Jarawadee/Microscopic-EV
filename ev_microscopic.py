...
def content_diagnosis_method():
    st.header("🩺 วิธีการตรวจหาพยาธิเข็มหมุด")
    
    st.markdown("""
    วิธีมาตรฐานทางการแพทย์ที่นิยมใช้และแม่นยำที่สุดคือ **Scotch Tape Technique** เนื่องจากพยาธิเข็มหมุดมักไม่ปะปนออกมากับอุจจาระ แต่จะวางไข่ติดอยู่บริเวณรอบทวารหนัก
    """)

    # สร้างกรอบเน้นย้ำเรื่องเวลา (สำคัญมาก)
    st.info("🕒 **Tip:** ช่วงเวลาที่เหมาะสมที่สุดในการตรวจคือ **'ตอนเช้าหลังตื่นนอนทันที'** (ก่อนอาบน้ำหรือขับถ่าย)")

    st.subheader("ขั้นตอนการเก็บตัวอย่าง (Scotch Tape Technique)")
    
    # แบ่งเป็น 3 คอลัมน์ ใส่ Icon ให้ดูง่าย
    col1, col2, col3 = st.columns(3)
    
    with col1:
        st.markdown("### 1. เตรียมอุปกรณ์")
        st.markdown("ใช้ **เทปใส (Clear Tape)** และแผ่นกระจกสไลด์")
        st.caption("🚫 ห้ามใช้เทปขุ่น")
    
    with col2:
        st.markdown("### 2. แปะเทป")
        st.markdown("กดด้านกาวลงบน **ผิวหนังรอบทวารหนัก**")
        st.caption("ทำซ้ำๆ ให้ทั่วบริเวณ")
        
    with col3:
        st.markdown("### 3. ปิดผนึก")
        st.markdown("นำเทปแปะลงบน **กระจกสไลด์** ให้เรียบ")
        st.caption("เตรียมนำไปส่องกล้อง")

    st.divider()
    
    st.markdown("### 🖼️ ตัวอย่างลักษณะไข่พยาธิ")
    st.markdown("""
    เมื่อนำสไลด์ไปส่องผ่านกล้องจุลทรรศน์ จะพบไข่พยาธิลักษณะเฉพาะ:
    * 🥚 **รูปร่าง:** รี ยาว ปลายมน (รูปตัว D หรือ เมล็ดข้าวสาร)
    * ⚪ **สี:** ใส ไม่มีสี
    * 📏 **ขนาด:** ประมาณ 50-60 ไมครอน
    """)
    
    # ตรงนี้ถ้ามีรูปตัวอย่างไข่พยาธิของจริง ใส่เปรียบเทียบจะดีมาก
    # st.image("egg_example.jpg", caption="ลักษณะไข่พยาธิเข็มหมุดภายใต้กล้องจุลทรรศน์")

# --- วิธีเรียกใช้ในหน้า Knowledge ---
# with tab_diagnosis: (สมมติว่าคุณสร้าง tab ใหม่)
#    content_diagnosis_method()
